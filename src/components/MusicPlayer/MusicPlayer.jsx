...
//   const { width, loggedInUser } = ContextProvider();
...
//   if (!loggedInUser.status || musicPlayer != "active") {
//     return null;
//   }
...
function MusicPlayer() {
  const { width, loggedInUser } = ContextProvider();
  const { musicPlayer, musicId, musicStatus, musicDispatch } = useMusic();
  const [music, setMusic] = useState({});
  ...
  const artistArray = music?.artist?.map((eItem) => eItem.name).slice(0, 2);
  ...
  return (
    <div className="musicPlayerContainer">
      ...
      {width > 709 && (
        <div className="musicMiddle">
          <div className="musicPlayerIcons">
            <SkipPreviousIcon style={{ fontSize: "2rem", opacity: 0.5 }} />
            <Replay10Icon style={{ fontSize: "2rem", opacity: 0.5 }} />
            {musicStatus !== "play" ? (
              <PlayArrowIcon
                onClick={handlePlay}
                style={{ fontSize: "3rem", cursor: "pointer" }}
            ...
            <Forward10Icon style={{ fontSize: "2rem", opacity: 0.5 }} />
            <SkipNextIcon style={{ fontSize: "2rem", opacity: 0.5 }} />
          </div>
        </div>
      )}
