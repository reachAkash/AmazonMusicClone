...
export default function MusicContainer({ musicData }) {
  ...
  return (
    <div className={`container ${backColor}`}>
      <div className="containerHeader items-center justify-between">
        <div className="title md:text-sm">{title}</div>
        <div className="seeAllItems">
          <div className="sliderIcons">
            <KeyboardArrowLeftIcon
              className="sliderLeftIcon"
              disabled={isLeft}
              style={{
                opacity: isLeft && "0.5",
                color: isLeft && "grey",
          ...
          <button className={`seeAllBtn`} onClick={handleSeeAll}>
            SEE ALL
          </button>
        </div>
      </div>
